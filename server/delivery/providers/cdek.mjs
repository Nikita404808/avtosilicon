...
export async function calculate(options) {
  const { type, total_weight, pickup_point_id, address } = options ?? {};
  ...
  const deliveryPrice = Number(data.total_sum ?? data.price ?? data.delivery_sum ?? 0);
  const currency = data.currency ?? 'RUB';
  const periodMin = data.period_min ?? data.period?.min;
  const periodMax = data.period_max ?? data.period?.max;
  const eta =
